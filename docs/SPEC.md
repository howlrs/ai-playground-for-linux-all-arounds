...
| ツール | 用途 |
|--------|------|
| SurrealDB v3.x (`surreal`) | マルチエージェント共有DB (AgentDB) |
| agentdb CLI | SurrealDB CLIラッパー — ログ記録・知見検索・グラフ関係 |

AgentDB は Namespace `agents` 配下に2つのデータベースを持つ:
- **logs** — イベントログ（TTL 14日、cron日次削除）
- **knowledge** — 知見・意思決定の永続ストア
...
| msgraph-sdk | PyPI (uv add) | Microsoft Graph API ※プロジェクト依存として追加 |
| azure-identity | PyPI (uv add) | Azure認証 ※プロジェクト依存として追加 |
...
| `agentdb-data` | `/home/dev/.agentdb` | AgentDB (SurrealDB) データ |
...
├── .gitignore              # Git除外設定
├── CLAUDE.md               # Claude Code行動指針
├── AGENTS.md               # エージェント連携定義
├── GEMINI.md               # Gemini CLI QAガイド
├── scripts/
│   ├── setup-all.sh        # 一括セットアップ（AgentDB起動含む）
...
│   ├── start-surreal.sh    # AgentDB (SurrealDB) 起動・スキーマ初期化
│   ├── init-schema.surql   # AgentDB スキーマ定義
│   ├── agentdb             # AgentDB CLIラッパー
│   └── agentdb-cleanup.sh  # AgentDB TTLクリーンアップ (cron)
├── docs/
│   ├── SPEC.md             # 本仕様書
│   ├── DESCRIPTION.md      # 説明書
│   ├── USAGE.md            # 利用書
│   ├── CAUTION.md          # 外部サービス注意事項
│   └── AGENTDB.md          # AgentDB活用方針
└── examples/
    ├── projects/            # プロジェクトテンプレート
    └── scenarios/           # ワークフローシナリオ
```
